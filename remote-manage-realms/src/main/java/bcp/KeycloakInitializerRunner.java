...
public class KeycloakInitializerRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<UserRepresentation> userRepresentations = MOVIES_APP_USERS.stream().map(userPass -> {
            ...
            if ("manager".equals(userPass.getUsername())) {
        ...
        // Testing
        UserPass manager = MOVIES_APP_USERS.get(0);
        log.info("Testing getting token for '{}' ...", manager.getUsername());

        Keycloak keycloakMovieApp = KeycloakBuilder.builder().serverUrl(keycloakServerUrl)
                .realm(COMPANY_SERVICE_REALM_NAME).username(manager.getUsername()).password(manager.getPassword())
                .clientId(MOVIES_APP_CLIENT_ID).build();

        log.info("'{}' token: {}", manager.getUsername(), keycloakMovieApp.tokenManager().grantToken().getToken());
        log.info("'{}' initialization completed successfully!", COMPANY_SERVICE_REALM_NAME);
    }

    private static final String COMPANY_SERVICE_REALM_NAME = "my-tenant";
    private static final String MOVIES_APP_CLIENT_ID = "tenant-app";
    private static final List<String> MOVIES_APP_ROLES = Arrays.asList("USER",
            "MOVIES_MANAGER");
    private static final String MOVIES_APP_REDIRECT_URL = "http://localhost:3000/*";
    private static final List<UserPass> MOVIES_APP_USERS = Arrays.asList(new UserPass("manager", "manager"),
